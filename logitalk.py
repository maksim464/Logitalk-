...
class MainWindow(CTk):
   def __init__(self):
       ...
       self.chat_field = CTkTextbox(self.main_frame, state="disabled", text_color="#33ff33")
       ...
       self.emoji_btn = CTkButton(self.bottom_frame, text="🔥", width=35, command=self.add_emoji)
       ...
       self.send_button = CTkButton(self.bottom_frame, text="Надіслати", width=70, command=self.send_message)
       ...
       try:
           ...
           self.title(f"Chat — ● Online ({self.username})")
   ...
   def toggle_menu(self):
       if not self.is_show_menu:
           ...
           self.name_label = CTkLabel(self.menu_frame, text="Налаштування")
           self.name_label.pack(pady=10)
           self.name_entry = CTkEntry(self.menu_frame, placeholder_text="Твоє ім'я")
   ...
   def save_name(self):
       ...
       if new_name:
           ...
           self.title(f"Chat — ● Online ({self.username})")
           self.add_message(f"[SYSTEM] Ім'я змінено: {old_name} -> {new_name}")


   def add_message(self, text):
       ### ПРИКОЛЯС: Додаємо час до кожного повідомлення
       current_time = datetime.now().strftime("%H:%M")
       self.chat_field.configure(state="normal")
       self.chat_field.insert("end", f"[{current_time}] {text}\n")
